...
parser.add_argument('--algorithm', type=str, default='erm',
                    help='training scheme, choose between fish or erm, ada.')
...
# ada arguments
parser.add_argument("--anchorlevels", default=24, type=int)
...
train_loader, tv_loaders = modelC.getDataLoaders(args, device=device, create_anchors=args.algorithm == 'ada', anchor_levels=args.anchorlevels)
...
def train_ada(train_loader, epoch, agg):
    gammas = get_gammas(args.alpha, 20)

    print('into train_ada')
    model.train()
    train_loader.dataset.reset_batch()
    print('\n====> Epoch: {:03d} '.format(epoch))
    ...
    for i, data in enumerate(train_loader):
        ...
        #print(anchors)
        #print(anchors.shape)
        #idx = np.nonzero(np.all(anchors[..., :] == 0, axis=0))
        idx  = torch.nonzero(torch.all(anchors[..., :] == 0, dim=0))
        anchors = torch.tensor(np.delete(anchors.cpu().numpy(), idx.cpu().numpy(), axis=1))
        gamma = gammas[np.random.randint(0, len(gammas))]
        anchor_X, anchor_y = ADA.transform_pytorch(X=x, y=y, gamma=gamma, anchorMatrix=anchors)
        anchor_X = anchor_X.float().to(device)
        anchor_y = anchor_y.float().to(device)

        optimiserC.zero_grad()

        y_hat = model(anchor_X)
        loss = criterion(y_hat, anchor_y)
        print(loss)
        loss.backward()
        #if args.use_bert_params:
        #    torch.nn.utils.clip_grad_norm_(model.parameters(),
...
if __name__ == '__main__':
    ...
    # generate global clustering in data loaders ideally..? 
