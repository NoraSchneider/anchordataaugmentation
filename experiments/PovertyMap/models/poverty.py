...
from ada import clustered_anchor
...
class Poverty_Batched_Dataset(Dataset):
    ...
    """
    def __init__(self, dataset, split, batch_size, transform=None, args = None, anchors=False):
        ...
        self.anchors = None
        if anchors:
            self.anchors = np.load(f"anchors_{args.fold}.npy")
    ...
    def __getitem__(self, idx):
        if self.anchors is not None:
            return self.transform(self.get_input(idx)), \
                self.targets[idx], self.domains[idx], idx, self.anchors[self.split_idx[idx],:]
        else:
            return self.transform(self.get_input(idx)), \
...
class Model(nn.Module):
    ...
    @staticmethod
    def getDataLoaders(args, device, create_anchors=True, anchor_levels=24):
        kwargs = {'no_nl': False,
                  'fold': args.fold,
        ...
        if create_anchors:
            ...
            for i in range(len(dataset)):
                all_ys.append(dataset[i][1].item())
            anchors = clustered_anchor(X=np.array(all_ys).reshape(-1,1),anchor_levels=anchor_levels)
            np.save(f"anchors_{args.fold}.npy", anchors)

        # get all train data
        transform = initialize_poverty_train_transform()

        train_sets = Poverty_Batched_Dataset(dataset, 'train', args.batch_size, transform, args,True)
        datasets = {}
